...
<section class="panel">
	<header class="panel-heading">
		<div class="panel-actions">
			<?php if (get_permission('catalog_rule', 'is_edit')): ?>
				<!-- POST, not a link: reindexing mutates the price index, so it must
				     not be triggerable by a crawler or prefetcher. The CSRF field is
				     injected into method="post" forms by MY_Controller::_output(). -->
				<form action="<?php echo base_url('catalog_rule/reindex'); ?>" method="post" style="display:inline-block;margin:0">
					<button type="submit" class="btn btn-default btn-sm">
						<i class="fas fa-sync-alt"></i> <?php echo translate('reindex_prices') ?: 'Reindex prices'; ?>
					</button>
				</form>
			<?php endif; ?>
			<?php if (get_permission('catalog_rule', 'is_add')): ?>
				<a href="<?php echo base_url('catalog_rule/create'); ?>" class="btn btn-primary btn-sm">
					<i class="fas fa-plus-circle"></i> <?php echo translate('add_catalog_rule') ?: 'Add Catalog Price Rule'; ?>
				</a>
			<?php endif; ?>
		</div>
		<h2 class="panel-title"><i class="fas fa-percentage"></i> <?php echo translate('catalog_price_rules') ?: 'Catalog Price Rules'; ?></h2>
	</header>
	<div class="panel-body">
				<table class="table table-bordered table-hover table-condensed" cellspacing="0" width="100%" id="catalog-rule-table">
			<thead>
				<tr>
					<th><?php echo translate('sl') ?: 'SL'; ?></th>
					<th><?php echo translate('name'); ?></th>
					<th><?php echo translate('scope') ?: 'Scope'; ?></th>
					<th><?php echo translate('action'); ?></th>
					<th><?php echo translate('discount') ?: 'Discount'; ?></th>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		var table = $('#catalog-rule-table').DataTable({
			"processing": true,
			"serverSide": true,
			"ordering": true,
			"order": [[1, "asc"]],
			...
			"ajax": {
				"url": "<?php echo base_url('catalog_rule/get_catalog_rules_server_side'); ?>",
				"type": "POST",
				"data": function (d) {
					d[csrfName] = csrfHash;
		...
		$(document).on('click', '.btn-status-toggle', function () {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo base_url('catalog_rule/status'); ?>",
				data: { id: btn.data('id') },
				dataType: 'json',

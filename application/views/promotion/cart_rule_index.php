...
<section class="panel">
	<header class="panel-heading">
		<div class="panel-actions">
			<?php if (get_permission('cart_rule', 'is_add')): ?>
				<a href="<?php echo base_url('cart_rule/create'); ?>" class="btn btn-primary btn-sm">
					<i class="fas fa-plus-circle"></i> <?php echo translate('add_cart_rule') ?: 'Add Cart Price Rule'; ?>
				</a>
			<?php endif; ?>
		</div>
		<h2 class="panel-title"><i class="fas fa-percent"></i> <?php echo translate('cart_price_rules') ?: 'Cart Price Rules'; ?></h2>
	</header>
	<div class="panel-body">
				<table class="table table-bordered table-hover table-condensed" cellspacing="0" width="100%" id="cart-rule-table">
			<thead>
				<tr>
					...
					<th><?php echo translate('min_subtotal') ?: 'Min Subtotal'; ?></th>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		var table = $('#cart-rule-table').DataTable({
			"processing": true,
			"serverSide": true,
			"ordering": true,
			"order": [[1, "asc"]],
			...
			"ajax": {
				"url": "<?php echo base_url('cart_rule/get_cart_rules_server_side'); ?>",
				"type": "POST",
				"data": function (d) {
					d[csrfName] = csrfHash;
		...
		$(document).on('click', '.btn-status-toggle', function () {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo base_url('cart_rule/status'); ?>",
				data: { id: btn.data('id') },
				dataType: 'json',

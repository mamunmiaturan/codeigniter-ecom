...
<section class="panel">
	<header class="panel-heading">
		<div class="panel-actions">
			<?php if (get_permission('inventory_source', 'is_add')): ?>
				<a href="<?php echo base_url('inventory_source/create'); ?>" class="btn btn-primary btn-sm">
					<i class="fas fa-plus-circle"></i> <?php echo translate('add_inventory_source') ?: 'Add Inventory Source'; ?>
				</a>
			<?php endif; ?>
		</div>
		<h2 class="panel-title"><i class="fas fa-warehouse"></i> <?php echo translate('inventory_sources') ?: 'Inventory Sources'; ?></h2>
	</header>
	<div class="panel-body">
				<table class="table table-bordered table-hover table-condensed" cellspacing="0" width="100%" id="inventory-source-table">
			<thead>
				<tr>
					<th><?php echo translate('sl') ?: 'SL'; ?></th>
					<th><?php echo translate('name'); ?></th>
					<th><?php echo translate('priority') ?: 'Priority'; ?></th>
					<th><?php echo translate('location') ?: 'Location'; ?></th>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		var table = $('#inventory-source-table').DataTable({
			"processing": true,
			"serverSide": true,
			"ordering": true,
			"order": [[2, "asc"]],
			...
			"ajax": {
				"url": "<?php echo base_url('inventory_source/get_inventory_sources_server_side'); ?>",
				"type": "POST",
				"data": function (d) {
					d[csrfName] = csrfHash;
		...
		$(document).on('click', '.btn-status-toggle', function () {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo base_url('inventory_source/status'); ?>",
				data: { id: btn.data('id') },
				dataType: 'json',

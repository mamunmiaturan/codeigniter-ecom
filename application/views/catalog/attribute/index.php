...
<section class="panel">
	<header class="panel-heading">
		<div class="panel-actions">
			<?php if (get_permission('attribute', 'is_add')): ?>
				<a href="<?php echo base_url('attribute/create'); ?>" class="btn btn-primary btn-sm">
					<i class="fas fa-plus-circle"></i> <?php echo translate('add_attribute') ?: 'Add Attribute'; ?>
				</a>
			<?php endif; ?>
		</div>
		<h2 class="panel-title"><i class="fas fa-tags"></i> <?php echo translate('attributes') ?: 'Attributes'; ?></h2>
	</header>
	<div class="panel-body">
				<table class="table table-bordered table-hover table-condensed" cellspacing="0" width="100%" id="attribute-table">
			<thead>
				<tr>
					<th><?php echo translate('sl') ?: 'SL'; ?></th>
					<th><?php echo translate('name'); ?></th>
					<th><?php echo translate('type') ?: 'Type'; ?></th>
					<th><?php echo translate('filterable') ?: 'Filterable'; ?></th>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		var table = $('#attribute-table').DataTable({
			...
			"ajax": {
				"url": "<?php echo base_url('attribute/get_attributes_server_side'); ?>",
				"type": "POST",
				"data": function (d) {
					d[csrfName] = csrfHash;
		...
		$(document).on('click', '.btn-status-toggle', function () {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo base_url('attribute/status'); ?>",
				data: { id: btn.data('id') },
				dataType: 'json',

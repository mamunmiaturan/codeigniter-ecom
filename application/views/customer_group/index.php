...
<section class="panel">
	<header class="panel-heading">
		<div class="panel-actions">
			<?php if (get_permission('customer_group', 'is_add')): ?>
				<a href="<?php echo base_url('customer_group/create'); ?>" class="btn btn-primary btn-sm">
					<i class="fas fa-plus-circle"></i> <?php echo translate('add_customer_group') ?: 'Add Customer Group'; ?>
				</a>
			<?php endif; ?>
		</div>
		<h2 class="panel-title"><i class="fas fa-users"></i> <?php echo translate('customer_groups') ?: 'Customer Groups'; ?></h2>
	</header>
	<div class="panel-body">
				<table class="table table-bordered table-hover table-condensed" cellspacing="0" width="100%" id="customer-group-table">
			<thead>
				<tr>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		var table = $('#customer-group-table').DataTable({
			"processing": true,
			"serverSide": true,
			"ordering": true,
			"order": [[1, "asc"]],
			...
			"ajax": {
				"url": "<?php echo base_url('customer_group/get_customer_groups_server_side'); ?>",
				"type": "POST",
				"data": function (d) {
					d[csrfName] = csrfHash;
		...
		$(document).on('click', '.btn-status-toggle', function () {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo base_url('customer_group/status'); ?>",
				data: { id: btn.data('id') },
				dataType: 'json',

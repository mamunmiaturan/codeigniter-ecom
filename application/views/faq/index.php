...
<section class="panel">
	<header class="panel-heading">
		<div class="panel-actions">
			<?php if (get_permission('faq', 'is_add')): ?>
				<a href="<?php echo base_url('faq/create'); ?>" class="btn btn-primary btn-sm"><i class="fas fa-plus-circle"></i> <?php echo translate('add_faq') ?: 'Add FAQ'; ?></a>
			<?php endif; ?>
		</div>
		<h2 class="panel-title"><i class="fas fa-question-circle"></i> <?php echo translate('faqs') ?: 'FAQs'; ?></h2>
	</header>
	<div class="panel-body">
				<table class="table table-bordered table-hover table-condensed" cellspacing="0" width="100%" id="faq-table">
			<thead>
				<tr>
					<th><?php echo translate('sl') ?: 'SL'; ?></th>
					<th><?php echo translate('question') ?: 'Question'; ?></th>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		var table = $('#faq-table').DataTable({
			...
			"columnDefs": [ {"targets":[0,4],"orderable":false}, {"targets":[3,4],"className":"text-center"} ],
			"ajax": {
				"url": "<?php echo base_url('faq/get_faqs_server_side'); ?>", "type": "POST",
		...
		$(document).on('click', '.btn-status-toggle', function () {
			var btn = $(this);
			$.ajax({ type: 'POST', url: "<?php echo base_url('faq/status'); ?>", data: { id: btn.data('id') }, dataType: 'json',

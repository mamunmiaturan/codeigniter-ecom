...
<section class="panel">
	<header class="panel-heading">
		<div class="panel-actions">
			<?php if (get_permission('cms', 'is_add')): ?>
				<a href="<?php echo base_url('cms/create'); ?>" class="btn btn-primary btn-sm"><i class="fas fa-plus-circle"></i> <?php echo translate('add_page') ?: 'Add Page'; ?></a>
			<?php endif; ?>
		</div>
		<h2 class="panel-title"><i class="fas fa-file-alt"></i> <?php echo translate('cms_pages') ?: 'CMS Pages'; ?></h2>
	</header>
	<div class="panel-body">
				<table class="table table-bordered table-hover table-condensed" cellspacing="0" width="100%" id="cms-table">
			<thead>
				<tr>
					<th><?php echo translate('sl') ?: 'SL'; ?></th>
					<th><?php echo translate('title') ?: 'Title'; ?></th>
					<th><?php echo translate('slug') ?: 'Slug'; ?></th>
					<th><?php echo translate('footer') ?: 'Footer'; ?></th>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		var table = $('#cms-table').DataTable({
			...
			"order": [[1, "asc"]], "pageLength": 25, "autoWidth": false,
			"columns": [ {"data":0},{"data":1},{"data":2},{"data":3},{"data":4},{"data":5} ],
			"columnDefs": [ {"targets":[0,5],"orderable":false}, {"targets":[3,4,5],"className":"text-center"} ],
			"ajax": {
				"url": "<?php echo base_url('cms/get_pages_server_side'); ?>", "type": "POST",
		...
		$(document).on('click', '.btn-status-toggle', function () {
			var btn = $(this);
			$.ajax({ type: 'POST', url: "<?php echo base_url('cms/status'); ?>", data: { id: btn.data('id') }, dataType: 'json',
